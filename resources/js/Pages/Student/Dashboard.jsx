...
import ActivityLogs from './Components/ActivityLogs'; // Pastikan path benar

export default function Dashboard({ auth, myClass, myGroup, tasks, logs }) {
    ...
    const updateStatus = (taskId, newStatus) => {
        const { post } = useForm();
        post(route('student.task.update-status', taskId), { 
    ...
    const handleComplete = (taskId) => {
        ...
        if (link) {
            const { post } = useForm();
            post(route('student.task.complete', taskId), { data: { link_evidence: link } });
        }
    };
    ...
    const handleDelete = (taskId) => {
        if(confirm('Hapus tugas ini?')) {
            ...
            destroy(route('student.task.delete', taskId));
        }
    };

    // --- SUB-COMPONENT: KANBAN COLUMN (KOMBINASI PUTIH BERSIH) ---
                    ...
                    <div key={task.id} className="group bg-slate-50 border border-slate-200 p-6 rounded-[2rem] hover:bg-white hover:border-blue-400 hover:shadow-xl transition-all duration-300">
                        ...
                        <div className="flex justify-between items-center mt-8">
                            <div className="flex gap-4">
                                ...
                                {status !== 'DONE' && (
                                    <button onClick={() => updateStatus(task.id, status === 'BACKLOG' ? 'IN_PROGRESS' : 'DONE')} className="text-[9px] font-black text-blue-600 hover:text-blue-800 uppercase tracking-tighter transition-colors font-black">Next →</button>
                                )}
                            </div>
                            {status === 'IN_PROGRESS' && (
            ...
            <div className="min-h-screen bg-[#0b1120] flex items-center justify-center p-6">
                ...
                <div className="max-w-md w-full bg-white p-12 rounded-[3.5rem] shadow-2xl text-center border border-slate-100">
                    ...
                    <form onSubmit={submitJoin} className="space-y-6">
                        <input 
                            type="text" 
                            className="w-full p-6 bg-slate-50 border-none rounded-3xl text-slate-800 text-center font-black tracking-[0.5em] focus:ring-2 focus:ring-blue-600 outline-none text-xl"
                            placeholder="XXXXXX"
                            value={joinForm.data.invite_code}
                            onChange={e => joinForm.setData('invite_code', e.target.value.toUpperCase())}
                            maxLength="6"
                        />
                        <button className="w-full py-5 bg-blue-600 text-white rounded-3xl font-black text-xs uppercase tracking-[0.2em] shadow-xl shadow-blue-600/30 active:scale-95 transition-all">
                            Gabung Sekarang
                        </button>
                    </form>
                </div>
    ...
    // --- VIEW 2: SUDAH JOIN TAPI BELUM ADA KELOMPOK (PUTIH BERSIH) ---
    if (!myGroup) {
        return (
    ...
    // --- VIEW 3: DASHBOARD UTAMA (KOMBINASI DARK-LIGHT) ---
    return (
        <AuthenticatedLayout user={auth.user}>
            <Head title="Student Intelligence Dashboard" />
            <div className="min-h-screen bg-[#0b1120] text-slate-300 pb-20 font-sans">
                <div className="max-w-[1600px] mx-auto px-6 sm:px-8 lg:px-10 pt-10 space-y-12">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-end gap-6 bg-slate-900/50 backdrop-blur-md p-12 rounded-[4rem] border border-slate-800 shadow-2xl">
                        <div className="space-y-4">
                            ...
                            <h1 className="text-6xl font-black text-white tracking-tighter italic leading-none">{myGroup.nama}</h1>
                            <p className="text-slate-500 font-bold text-lg">Fokus Proyek: <span className="text-slate-300 italic">"{myGroup.proyek || 'Belum ada judul'}"</span></p>
                        </div>
                        <div className="flex gap-10">
                            <button 
                                onClick={() => setTab('kanban')} 
                                className={`pb-4 text-xs font-black uppercase tracking-[0.3em] transition-all ${tab === 'kanban' ? 'text-blue-500 border-b-4 border-blue-600' : 'text-slate-600 hover:text-slate-400'}`}
                            >
                                Kanban Board
                            </button>
                            <button 
                                onClick={() => setTab('logs')} 
                                className={`pb-4 text-xs font-black uppercase tracking-[0.3em] transition-all ${tab === 'logs' ? 'text-emerald-500 border-b-4 border-emerald-600' : 'text-slate-600 hover:text-slate-400'}`}
                            >
                                Tim Logbook
                            </button>
                        </div>
                    ...
                    {/* Main Content: Kanban atau Logbook */}
            ...
            {/* Modal Tambah Tugas - White Pop Up */}
            {isTaskModalOpen && (
                        ...
                        <form onSubmit={submitTask} className="space-y-8">
                            <div className="space-y-3">
                                ...
                                <input 
                                    ...
                                    className="w-full bg-slate-50 border-none rounded-3xl text-sm text-slate-900 p-6 focus:ring-2 focus:ring-blue-600 transition-all outline-none"
                            ...
                            <div className="flex gap-6 pt-6">
                                ...
                                <button type="submit" className="flex-1 py-6 bg-blue-600 text-white rounded-[2rem] font-black text-[10px] uppercase tracking-widest shadow-xl shadow-blue-600/30 active:scale-95 transition-all">Simpan Tugas</button>
                            </div>
                        </form>
                    </div>
                </div>
