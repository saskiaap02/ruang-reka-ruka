    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-slate-100/50 p-4 rounded-3xl min-h-[400px]">
                <h3 className="font-bold text-slate-500 mb-4 tracking-widest text-xs uppercase">Backlog</h3>
            ...
            {/* Kolom In Progress */}
            <div className="bg-slate-100/50 p-4 rounded-3xl min-h-[400px]">
                <h3 className="font-bold text-blue-500 mb-4 tracking-widest text-xs uppercase">In Progress</h3>
                {tasks?.filter(t => t.status === 'in_progress').map(task => (
                    <div key={task.id} className="bg-white p-4 rounded-2xl mb-3 shadow-sm border-l-4 border-l-blue-500">
                        <h4 className="font-bold text-sm text-slate-800">{task.judul}</h4>

                        {/* Tampilan PIC (Penanggung Jawab) */}
                        <div className="flex items-center gap-2 mt-2">
                            <div className="w-5 h-5 bg-blue-100 rounded-full flex items-center justify-center text-[8px] font-bold text-blue-600">
                                {task.pic_name ? task.pic_name.charAt(0) : '?'}
                            </div>
                            <span className="text-[10px] text-slate-400 font-medium">PIC: {task.pic_name || 'Misterius'}</span>
                        </div>
            ...
            {/* Kolom Done */}
            <div className="bg-slate-100/50 p-4 rounded-3xl min-h-[400px]">
                <h3 className="font-bold text-emerald-500 mb-4 tracking-widest text-xs uppercase">Done</h3>
