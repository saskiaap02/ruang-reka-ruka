...
function StudentAuditRow({ kelompok, student }) {
    const [nilaiDasar, setNilaiDasar] = useState(student.nilai_dasar || 85);
    ...
    const totalAkhir = ((Number(nilaiDasar) * bobotDasar) + (Number(nilaiAudit) * bobotAudit) + (Number(peerScore) * bobotPeer)).toFixed(2);
    ...
    return (
        <tr className="border-b border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
            ...
            <td className="p-4 text-center">
                <input type="number" min="0" max="100" value={nilaiDasar} onChange={(e) => setNilaiDasar(e.target.value)} className="w-16 text-center border border-slate-200 dark:border-slate-700 dark:bg-slate-900 rounded-lg py-1 text-sm font-semibold text-slate-600 dark:text-slate-300 focus:ring-2 focus:ring-blue-500 outline-none" />
            </td>
            <td className="p-4 text-center">
                <div className="flex items-center justify-center gap-2">
                    <input type="number" min="0" max="100" value={nilaiAudit} onChange={(e) => setNilaiAudit(e.target.value)} className="w-16 text-center border border-slate-300 dark:border-slate-700 dark:bg-slate-900 rounded-lg py-1 text-sm font-bold text-slate-800 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none" />
                    <button onClick={handleSave} disabled={isSaving} className={`w-8 h-8 flex items-center justify-center rounded-lg transition-all ${isSaving ? 'bg-slate-300 cursor-not-allowed' : 'bg-emerald-500 hover:bg-emerald-600 text-white shadow-sm active:scale-95'}`} title="Simpan Nilai">
                        {isSaving ? '...' : <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>}
                    </button>
                </div>
            </td>
    ...
    return (
        <AuthenticatedLayout user={auth.user} header={
            <div className="flex items-center gap-4">
                <Link href={route('dosen.kelas.show', kelompok.project_class_id)} className="text-slate-400 hover:text-slate-600 transition-colors">
                ...
                <div>
                    <h2 className="font-semibold text-xl text-slate-800 dark:text-white leading-tight">Audit: {kelompok.nama_kelompok}</h2>
                    <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold mt-1">{kelompok.project_title || 'Judul Belum Ditentukan'}</p>
                </div>
            </div>
        }>
            ...
            <div className="py-12">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        {/* Panel Anggota */}
                        <div className="lg:col-span-1 bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                                ...
                                {anggota.map((user) => (
                                    <div key={user.id} className="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
                                        <div>
                                            ...
                                            <p className={`text-[10px] uppercase font-bold tracking-tighter mt-1 ${user.is_inactive ? 'text-red-500' : 'text-slate-500'}`}>Log Akhir: {user.last_activity}</p>
                                        </div>
                                        {user.is_inactive && (
                                            <button onClick={() => handleColek(user.id)} disabled={processing} className="bg-orange-500 hover:bg-orange-600 text-white px-3 py-1.5 rounded-lg text-[10px] font-black shadow-lg shadow-orange-200 transition-all active:scale-90">
                                                COLEK
                                            </button>
                                        )}
                                    </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {['backlog', 'in_progress', 'done'].map((status) => (
                                    <div key={status} className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800">
                                        <p className="text-[10px] uppercase font-black text-slate-400 mb-3 tracking-widest border-b border-slate-200 dark:border-slate-700 pb-2">{status.replace('_', ' ')}</p>
                                        <div className="space-y-2">
                                            ...
                                            {tasks.filter(t => t.status === status).length === 0 && <p className="text-[10px] italic text-slate-400 text-center py-2">Tidak ada tugas</p>}
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                        <div className="p-6 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex justify-between items-center">
                            <h3 className="font-bold text-slate-800 dark:text-white flex items-center gap-2">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4 text-emerald-500"><path strokeLinecap="round" strokeLinejoin="round" d="m3.75 13.5 10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75Z" /></svg>
                                Log Aktivitas (Heatmap Jejak)
                            </h3>
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-white dark:bg-slate-900 px-3 py-1 rounded-full border border-slate-200 dark:border-slate-700">Real-time Audit</span>
                        </div>
                        <div className="p-8">
                            <div className="relative border-l-2 border-slate-200 dark:border-slate-700 ml-4 space-y-8">
                                {logs.length > 0 ? logs.map((log) => (
                                    <div key={log.id} className="relative pl-8">
                                        ...
                                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{new Date(log.created_at).toLocaleString('id-ID')}</p>
                                        <div className="mt-1 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 inline-block min-w-[300px]">
                                            ...
                                            <div className="mt-3 flex items-center gap-2">
                                                <span className="px-2 py-0.5 bg-blue-100 dark:bg-blue-900/40 text-[9px] font-black rounded text-blue-600 uppercase tracking-tighter">{log.action_type}</span>
                                            </div>
                                        </div>
                                    </div>
                                )) : (
                                    <div className="text-center py-10"><p className="text-slate-400 italic text-sm">Belum ada jejak aktivitas terdeteksi.</p></div>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    {/* Rekap Auto-Grading */}
                    <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden mt-6">
                        <div className="p-6 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex justify-between items-center">
                            <div>
                                <h3 className="font-bold text-slate-800 dark:text-white">Rekap Penilaian Akhir (Auto-Grading)</h3>
                                <p className="text-[10px] text-slate-500 mt-1 uppercase font-bold tracking-widest">Bobot: Dasar ({kelompok.bobot_dasar}%) | Audit ({kelompok.bobot_audit}%) | Peer ({kelompok.bobot_peer}%)</p>
                            </div>
                            <a href={route('dosen.kelas.ekspor', kelompok.project_class_id)} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-bold shadow-sm transition-all flex items-center gap-2 active:scale-95">
                        ...
                        <div className="p-0 overflow-x-auto">
                            <table className="w-full text-left text-sm">
                                ...
                                <tbody>
                                    {anggota && anggota.length > 0 ? anggota.map((student) => (
                                        <StudentAuditRow key={student.id} kelompok={kelompok} student={student} />
                                    )) : (
                                        <tr><td colSpan="5" className="p-12 text-center text-slate-400 text-sm font-medium">Belum ada anggota di kelompok ini.</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
